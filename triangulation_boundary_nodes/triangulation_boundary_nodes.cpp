...
//
//    int TRIANGLE_NUM, the number of triangles.
//
//    int TRIANGLE_NODE[3*TRIANGLE_NUM], the nodes that make up the
//    triangles.  These should be listed in counterclockwise order.
//
//  Output:
//
//    bool TRIANGULATION_ORDER3_BOUNDARY_NODE[NODE_NUM],
...
//
  ...
  while ( j < 3 * triangle_num )
  {
    ...
    if ( j == 3 * triangle_num )
    {
      for ( i = 0; i < m; i++ )
      {
        node_boundary[edge[i+(j-1)*m]] = true;
      }
      break;
    }
...
//    int TRIANGLE_NUM, the number of triangles.
//
//    int TRIANGLE_NODE[6*TRIANGLE_NUM], the nodes that make up the
//    triangles.
//
//  Output:
//
//    bool TRIANGULATION_ORDER6_BOUNDARY_NODE[NODE_NUM],
//    is TRUE if the node is on a boundary edge.
//
{
  int e1;
