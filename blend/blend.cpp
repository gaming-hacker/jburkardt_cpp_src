...
void blend_101 ( double r, double x0, double x1, double *x )
...
void blend_102 ( double r, double s, double x00, double x01, double x10, 
  double x11, double *x )
...
void blend_103 ( double r, double s, double t, double x000, double x001, 
  double x010, double x011, double x100, double x101, double x110, 
  double x111, double *x )
...
void blend_112 ( double r, double s, double x00, double x01, double x10,
...
void blend_113 ( double r, double s, double t, double x000, double x001, 
...
//
  blend_112 ( s, t, x000, x001, x010, x011, x0s0, x0s1, x00t, x01t, &x0st );
  blend_112 ( s, t, x100, x101, x110, x111, x1s0, x1s1, x10t, x11t, &x1st );
  blend_112 ( r, t, x000, x001, x100, x101, xr00, xr01, x00t, x10t, &xr0t );
  blend_112 ( r, t, x010, x011, x110, x111, xr10, xr11, x01t, x11t, &xr1t );
  blend_112 ( r, s, x000, x010, x100, x110, xr00, xr10, x0s0, x1s0, &xrs0 );
  blend_112 ( r, s, x001, x011, x101, x111, xr01, xr11, x0s1, x1s1, &xrs1 );
//
//  Interpolate the I-th coordinate component of the interior point.
//
  blend_123 ( r, s, t, x000, x001, x010, x011, x100, x101, x110, x111, 
...
void blend_123 ( double r, double s, double t,
...
//
  for ( i = 1; i < m1 - 1; i++ )
  {
    ...
    blend_101 ( r, x[0], x[m1*m2-m2], &x[i*m2] );

    blend_101 ( r, x[m2-1], x[m1*m2-1], &x[i*m2+m2-1] );
  }
  ...
  for ( j = 1; j < m2 - 1; j++ )
  {
    ...
    blend_101 ( s, x[0], x[m2-1], &x[j] );

    blend_101 ( s, x[m1*m2-m2], x[m1*m2-1], &x[(m1-1)*m2+j] );
  }
//
...
//
  for ( i = 1; i < m1 - 1; i++ )
  {
    ...
    for ( j = 1; j < m2 - 1; j++ )
    {
      ...
      blend_112 ( r, s, x[0], x[m2-1], x[m1*m2-m2], x[m1*m2-1],
...
//
  ...
  {
    r = ( double ) i / ( double ) ( m1 - 1 );
    blend_101 ( r, x[0],              x[(m1-1)*m3*m2],
                  &x[i*m3*m2] );

    blend_101 ( r, x[(m2-1)*m2],      x[((m1-1)*m3+m2-1)*m2],
                  &x[(i*m3+m2-1)*m2] );

    blend_101 ( r, x[m3-1],           x[(m1-1)*m3*m2+m3-1],
                  &x[i*m3*m2+m3-1] );

    blend_101 ( r, x[(m2-1)*m2+m3-1], x[((m1-1)*m3+m2-1)*m2+m3-1],
                  &x[(i*m3+m2-1)*m2+m3-1] );
  }
  ...
  {
    s = ( double ) j / ( double ) ( m2 - 1 );
    blend_101 ( s, x[0], x[(m2-1)*m2],
                  &x[j*m2] );

    blend_101 ( s, x[(m1-1)*m3*m2], x[((m1-1)*m3+m2-1)*m2],
                  &x[((m1-1)*m3+j)*m2] );

    blend_101 ( s, x[m3-1], x[(m2-1)*m2+m3-1],
                  &x[j*m2+m3-1] );

    blend_101 ( s, x[(m1-1)*m3*m2+m3-1], x[((m1-1)*m3+m2-1)*m2+m3-1],
                  &x[((m1-1)*m3+j)*m2+m3-1] );
  }
  ...
  for ( k = 1; k < m3 - 1; k++ )
  {
    ...
    blend_101 ( t, x[0], x[m3-1],
                  &x[k] );

    blend_101 ( t, x[(m1-1)*m3*m2], x[(m1-1)*m3*m2+m3-1],
                  &x[(m1-1)*m3*m2+k] );

    blend_101 ( t, x[(m2-1)*m2], x[(m2-1)*m2+m3-1],
                  &x[(m2-1)*m2+k] );

    blend_101 ( t, x[((m1-1)*m3+m2-1)*m2], x[((m1-1)*m3+m2-1)*m2+m3-1],
                  &x[((m1-1)*m3+m2-1)*m2+k] );
  }
//
...
//
    blend_101 ( r, x00, x10, &xr0 );
    blend_101 ( r, x01, x11, &xr1 );
    blend_101 ( s, x00, x01, &x0s );
    blend_101 ( s, x10, x11, &x1s );
//
//  Interpolate the I-th coordinate component of the interior point.
//
    blend_112 ( r, s, x00, x01, x10, x11, xr0, xr1, x0s, x1s, &x[i] );
...
//
    blend_101 ( r, x000, x100, &xr00 );
    blend_101 ( r, x001, x101, &xr01 );
    blend_101 ( r, x010, x110, &xr10 );
    blend_101 ( r, x011, x111, &xr11 );

    blend_101 ( s, x000, x010, &x0s0 );
    blend_101 ( s, x001, x011, &x0s1 );
    blend_101 ( s, x100, x110, &x1s0 );
    blend_101 ( s, x101, x111, &x1s1 );

    blend_101 ( t, x000, x001, &x00t );
    blend_101 ( t, x010, x011, &x01t );
    blend_101 ( t, x100, x101, &x10t );
    blend_101 ( t, x110, x111, &x11t );
//
...
//  Interpolate the I-th coordinate component of the interior point.
//
    blend_123 ( r, s, t, x000, x001, x010, x011, x100, x101,
