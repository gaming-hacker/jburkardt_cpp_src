...
//
  ...
  if ( 0 < lo ) 
  {
    ...
    for ( k = 1; k < mx; k++ )
    {
      emx = r8_max ( fabs ( e[k] ), emx );
      emn = r8_min ( fabs ( e[k] ), emn );
      erx = r8_max ( fabs ( er[k] ), erx );
      ern = r8_min ( fabs ( er[k] ), ern );
    }
...
//
{
  ...
  else if ( kind == 6 )
  {
    ...
    else
    {
      for ( i = 0; i < nt; i++ )
      {
        w[i] = exp ( - t[i] * t[i] ) * pow ( fabs ( t[i] ), alpha );
      }
    }
  }
  else if ( kind == 7 )
